...
class WusnNetwork(Network):
    ...
    def calc_max_energy_consumption_1(self):
    ...
    def transmission_energy(self, index, k, d): 
        if index > self.n:
            return k * WusnConstants.e_fs * (d ** 2)
        else:
            return k * WusnConstants.e_mp * (d ** 4)
    ...
    def calc_max_energy_consumption(self):
        ...
        for index in range(1, self.node_count):
            if self.parent[index] != -1:
                d = distance(self._points[index], self._points[self.parent[index]])
                if index > self.n : 
                    e = WusnConstants.k_bit * (WusnConstants.e_elec + WusnConstants.e_fs * d ** 2)
                else:
                    e = WusnConstants.k_bit * (self.num_childs[index] * (WusnConstants.e_elec + WusnConstants.e_da) + WusnConstants.e_mp * d ** 4)
